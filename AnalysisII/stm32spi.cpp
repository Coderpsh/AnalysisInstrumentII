...
int gpio_init(struct DeviceInfo *spi)
{
    ...
    if(gpio_set_value(spi->fd_rest,Low)<0){
		printf("init set value gpio %d error\n",spi->num_rest);
		return 0;
	}
	//初始化采样分辨率：
    if(gpio_set_value(spi->fd_os0,Low)<0){
		printf("init set value gpio %d error\n",spi->num_os0);
		return 0;
	}
    if(gpio_set_value(spi->fd_os1,Low)<0){
		printf("init set value gpio %d error\n",spi->num_os1);
		return 0;
	}
    if(gpio_set_value(spi->fd_os2,Low)<0){
		printf("init set value gpio %d error\n",spi->num_os2);
		return 0;
	}
	return 1;
...
int spi_init(struct DeviceInfo *spi)
{
	...
	//片选：拉低cs
    if(gpio_set_value(spi->fd_cs,Low)<0)
	{
...
int spi_read_data(struct DeviceInfo *spi,uint8_t *buf,int len)
{
	/*
	//转换信号：拉低cov
    if(gpio_set_value(spi->fd_cov,Low)<0){
...
//发送数据
int spi_write_data(struct DeviceInfo *spi,uint8_t *buf,int len)
{
	//片选：拉低cs
    // if(gpio_set_value(spi->fd_cs,Low)<0)
	// {
	...
	if(spi_fd_write(spi->fd_spi,buf,len)<0){
...
//全双工通信
//发送数据
int spi_write_tr(struct DeviceInfo *spi,uint8_t *tbuf,uint8_t *rbuf,int len)
{
	//片选：拉低cs
    // if(gpio_set_value(spi->fd_cs,Low)<0)
	// {
	...
	if(spi_fd_tr(spi->fd_spi,tbuf,rbuf,len)<0)  //spi.c
	{
		printf("tr data error\n");
		return 0;
	}
